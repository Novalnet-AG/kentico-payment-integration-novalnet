...
public class NovalnetGatewayProvider : CMSPaymentGatewayProvider, IDirectPaymentGatewayProvider
{
    ...
    {
        string vendor = SettingsKeyInfoProvider.GetValue("vendor_id");
        string authCode = SettingsKeyInfoProvider.GetValue("auth_code");
        string product = SettingsKeyInfoProvider.GetValue("product_id");
        string tariff = SettingsKeyInfoProvider.GetValue("tariff_id");
    ...
    public PaymentResultInfo ProcessPayment(IDictionary<string, object> paymentData)
    {
        ...
        string vendor = SettingsKeyInfoProvider.GetValue("vendor_id");
        string password = SettingsKeyInfoProvider.GetValue("payment_access_key");
        string authCode = EncryptString(SettingsKeyInfoProvider.GetValue("auth_code"), password, uniqid);
        string product = EncryptString(SettingsKeyInfoProvider.GetValue("product_id"), password, uniqid);
        string tariff = EncryptString(SettingsKeyInfoProvider.GetValue("tariff_id"), password, uniqid);                
        string testMode = EncryptString(((SettingsKeyInfoProvider.GetValue("test_mode") == "True") ? "1" : "0"), password, uniqid);
        ...
        string referrerId = SettingsKeyInfoProvider.GetValue("referrer_id");
        // Get the amount for on-hold process
        string onholdAmount = SettingsKeyInfoProvider.GetValue("on_hold_limit");
        ...
        // Credit Card
        bool cc3d = Convert.ToBoolean(SettingsKeyInfoProvider.GetValue("cc3d"));
        // Invoice 
        string invoiceDuedate = SettingsKeyInfoProvider.GetValue("invoice_due_date");
        // Barzahlen
        string cashPaymentDuedate = SettingsKeyInfoProvider.GetValue("slip_expiry_date");
        //Direct Debit SEPA
        string sepaDuedate = SettingsKeyInfoProvider.GetValue("sepa_due_date");     
        ...
        param.AppendFormat("&chosen_only={0}", "0");
        param.AppendFormat("&address_form={0}", "0");
        ...
        var gateway_timeout = SettingsKeyInfoProvider.GetValue("gateway_timeout");
        // Initiate payport request
        WebRequest request = WebRequest.Create("https://paygate.novalnet.de/paygate.jsp");
        request.Method = "POST";
