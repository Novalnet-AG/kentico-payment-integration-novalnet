...
public class NovalnetCallback :IHttpHandler
{
    ...
    public IDictionary<string, string> ValidateRequestParams(HttpContext context)
    {
        ...
        bool deactivateIP = SettingsKeyInfoProvider.GetBoolValue("deactivate_ip",SiteContext.CurrentSiteID);
    ...
    public void InitialLevelPayments(IDictionary<string, string> parameter, HttpContext context)
    {
        ...
        {
            if (parameter["tid_status"] == "100" && nntransHistory["CallbackAmount"]is DBNull)
            {
        ...
        else if(parameter["payment_type"] == "PRZELEWY24")
        {
            ...
            {
                if (nntransHistory["CallbackAmount"] is DBNull)
                {
                    string callbackComments = "Novalnet Callback Script executed successfully for the TID: " + parameter["tid"] + " with amount " + Convert.ToDecimal(amount)/100 + " " + currency + " on " + now + " .";
                    orderInfo.OrderIsPaid = true;
                    ConnectionHelper.ExecuteQuery("update com_order set orderispaid =  1,orderstatusid = " + orderStatus.PaymentOptionSucceededOrderStatusID + " where orderid = '" + nntransHistory["OrderNo"] + "'", null, QueryTypeEnum.SQLQuery);
    ...
    public void InsertCallbackComments(string data, OrderInfo orderInfo, string testMode = "")
    {
        if(testMode == "1")
        {
            ...
            testModeText.Value = ResHelper.GetString("custom.test_mode");
    ...
    public void ShowBankDetails(IDictionary<string,object> nntransHistory,OrderInfo orderInfo,IDictionary<string,string> data)
    {
        ...
        string product = data.ContainsKey("product_id") ? data["product_id"] : "";
        ...
        item.Value =  string.Format(ResHelper.GetString("custom.duedate_title"), dueDate) + " | "  + string.Format(ResHelper.GetString("custom.account_holder"), invoiceAccountHolder) + " | " + string.Format("IBAN: {0}", invoiceIBAN) + " | " + string.Format("BIC: {0}", invoiceBIC) + " | " + string.Format("Bank: {0} ", invoiceBankname) + " | " + string.Format(ResHelper.GetString("custom.amount"), amount + " " + nntransHistory["Currency"]) + " | " + ResHelper.GetString("custom.payment_reference_title") + " | " + string.Format(ResHelper.GetString("custom.payment_reference1"), "BNR-"+product+"-"+orderNo+"") + " | " + string.Format(ResHelper.GetString("custom.payment_reference2"), data["shop_tid"]);
    ...
    {
        bool enableEmail = SettingsKeyInfoProvider.GetBoolValue("callback_email",SiteContext.CurrentSiteID);
        string emailTo = SettingsKeyInfoProvider.GetValue("email_to",SiteContext.CurrentSiteID);
        string emailFrom = SettingsKeyInfoProvider.GetValue("CMSAdminEmailAddress",SiteContext.CurrentSiteID);
        string mailTo = "";
        string mailSubject = "";
        // Initialize message
        ...
        {
            mailTo = SettingsKeyInfoProvider.GetValue("email_to",SiteContext.CurrentSiteID);
        ...
        if (SettingsKeyInfoProvider.GetValue("email_bcc",SiteContext.CurrentSiteID) != "")
        {
            email.BccRecipients = SettingsKeyInfoProvider.GetValue("email_bcc",SiteContext.CurrentSiteID);
        }
        ...
        {
            ServerName = SettingsKeyInfoProvider.GetValue("CMSSMTPServer",SiteContext.CurrentSiteID),
            ServerUserName = SettingsKeyInfoProvider.GetValue("CMSSMTPServerUser",SiteContext.CurrentSiteID),
#pragma warning disable 618
            ServerPassword = EncryptionHelper.EncryptData(SettingsKeyInfoProvider.GetValue("CMSSMTPServerPassword",SiteContext.CurrentSiteID)),
#pragma warning restore 618
            ServerUseSSL = Convert.ToBoolean(SettingsKeyInfoProvider.GetValue("CMSUseSSL",SiteContext.CurrentSiteID))
        };
